 ...
 * @author xiaqiliang
 */
public class BubbleDragLayer extends FrameLayout implements ViewGroup.OnHierarchyChangeListener {
    /* 当前处理的拖动项 */
    ...
    @SuppressLint("NewApi")
    public BubbleDragLayer(Context context) {
        super(context);
    ...
    @SuppressLint("NewApi")
	public BubbleDragLayer(Context context, AttributeSet attrs) {
        super(context, attrs);
        setMotionEventSplittingEnabled(false);
        setChildrenDrawingOrderEnabled(true);
        setOnHierarchyChangeListener(this);
	...
	private void hanleActionUP(int x, int y) {
        ...
        int max = getMeasuredWidth()-SUtils.getDip(getContext(),40);

        int top =  mDragItem.getViewTop();
        if(x > max){
            x = max;
        }

        int min = SUtils.getDip(getContext(),0);
        if(x < min){
            x = min;
        }
        int left = x - mMotionDownX + mDragItem.getViewLeft();
        Logs.i("getMeasuredWidth:"+getMeasuredWidth()+",,"+getWidth()+",,,"+left);
	...
	public void handleMove(int x, int y) {
        ...
        moveX = x;
        int max = getMeasuredWidth()-SUtils.getDip(getContext(),40);
        Logs.i("with:"+getMeasuredWidth()+"max::"+max+",,,x"+x);
        if(x > max){
           x = max;
        }
        onMoveX(x);

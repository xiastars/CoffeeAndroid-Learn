 ...
 * @author Sai 支持自动翻页
 */
public class ConvenientBottomBanner<T> extends LinearLayout {
    ...
    public ConvenientBottomBanner(Context context) {
        super(context);
        init(context);
    }

    public ConvenientBottomBanner(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public ConvenientBottomBanner(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public ConvenientBottomBanner(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(Context context) {
        View hView = LayoutInflater.from(context).inflate(
                R.layout.include_viewpager2, this, true);
...
/*
    ...
    static class AdSwitchTask implements Runnable {
        ...
        private final WeakReference<ConvenientBottomBanner> reference;

        AdSwitchTask(ConvenientBottomBanner convenientBanner) {
            this.reference = new WeakReference<ConvenientBottomBanner>(convenientBanner);
        }

        @Override
        public void run() {
            ConvenientBottomBanner convenientBanner = reference.get();
...
/*        int page = viewPager.getCurrentItem() + 1;
    ...
    public ConvenientBottomBanner setPages(CBPageAdapter pageAdapter, List<T> datas) {
        this.mDatas = datas;
    ...
    public ConvenientBottomBanner setPages(CBViewHolderCreator holderCreator, List<T> datas) {
        this.mDatas = datas;
        if (pageAdapter == null) {
     ...
     * @param visible
     */
    public ConvenientBottomBanner setPointViewVisible(boolean visible) {
     ...
     */
    public ConvenientBottomBanner setPageIndicator(int[] page_indicatorId) {
        ...
        Logs.i("size:::" + size);
        int leftMargin = (SUtils.screenWidth - SUtils.getDip(getContext(), 10 * size + 10 * (size - 1))) / 2;
        for (int count = 0; count < size; count++) {
            ...
            params.width = SUtils.getDip(getContext(), 10);
            params.height = SUtils.getDip(getContext(), 10);
            params.leftMargin = leftMargin + SUtils.getDip(getContext(), 20 * count);
        }
    ...
    private void setPagerText(int curPage) {
        ...
        STextUtils.setSpannableView(content, tvPageIndicator, 0, (curPage + "").length(), getContext().getResources().getColor(R.color.white));
    }
     ...
     * @param page_indicatorId
     */
    public ConvenientBottomBanner setTextPageIndicator(int[] page_indicatorId) {
        loPageTurningPoint.removeAllViews();
        mPointViews.clear();
        this.page_indicatorId = page_indicatorId;
        if (mDatas == null) return this;
        ...
        setPagerText(1);
        tvPageIndicator.setBackgroundResource(R.drawable.so_black45);
        ...
        params.rightMargin = SUtils.getDip(getContext(), 15);
        params.width = SUtils.getDip(getContext(), 40);
        params.height = SUtils.getDip(getContext(), 20);
        tvPageIndicator.setTextSize(12);
     ...
     * @return
     */
    public ConvenientBottomBanner setPageIndicatorAlign(PageIndicatorAlign align) {
     ...
     * @return
     */
    public ConvenientBottomBanner startTurning(long autoTurningTime) {
        //如果是正在翻页的话先停掉
        if (turning) {
            stopTurning();
        }
     ...
     * @return
     */
    public ConvenientBottomBanner setPageTransformer(PageTransformer transformer) {
     ...
     * @return
     */
    public ConvenientBottomBanner setOnPageChangeListener(ViewPager.OnPageChangeListener onPageChangeListener) {
     ...
     */
    public ConvenientBottomBanner setOnItemClickListener(OnSimpleClickListener onItemClickListener) {

...
public class CustomViewAbove extends ViewGroup {
    ...
    private SlidingMenu.OnMenuDragListener mMenuDragListener;
    ...
    void setCurrentItemInternal(int item, boolean smoothScroll, boolean always, int velocity) {
        ...
        if (destX < 0)
            destX = -(screenWidth - context.getResources().getDimensionPixelSize(R.dimen.slidingmenu_offset));
        else if (destX > 0) destX = screenWidth;
        if (mMenuDragListener != null) mMenuDragListener.onDrag(destX, getScrollY());
        if (smoothScroll) {
    ...
    public void setOnMenuDragListener(SlidingMenu.OnMenuDragListener l) {
        mMenuDragListener = l;
    }
        ...
        if (dx == 0 && dy == 0) {
            ...
            if (isMenuOpen()) {
                if (mOpenedListener != null)
                    mOpenedListener.onOpened(dx < 0 ? true : false);
            } else {
...
//		addView(mContent);
    ...
    private void completeScroll() {
        ...
        if (needPopulate) {
            ...
            if (isMenuOpen()) {
                if (mOpenedListener != null)
                    mOpenedListener.onOpened(x < 0 ? true : false);
            } else {
                if (mClosedListener != null)
                    mClosedListener.onClosed();
            }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE:
                //动态里有Viewpager,临时写法
                ViewPager viewPager = checkHasViewPager(this);
                if (viewPager != null) {
                    if (viewPager instanceof CustomerViewPager) {
                        if (viewPager.getCurrentItem() != 0 && MainFragment.curFragmentIndex == 2) {
                            return false;
                        }
                    }
    ...
    private ViewPager checkHasViewPager(ViewGroup parent) {
        int count = parent.getChildCount();
        for (int i = 0; i < count; i++) {
            View view = parent.getChildAt(i);
            if (view instanceof ViewGroup) {
                if (view instanceof ViewPager) {
                    if (view instanceof CustomerViewPager) {
                        return (CustomerViewPager) view;
                    }
                ...
                ViewPager has = checkHasViewPager(newParent);
                if (has != null) {
                    return has;
                }
            }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    if (mMenuDragListener != null)
                        mMenuDragListener.onDrag((int) scrollX, getScrollY());
                }
                break;
